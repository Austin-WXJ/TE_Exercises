...
SELECT name, population surface, lifeexpectancy FROM country WHERE continent = 'Asia';
...
SELECT (gnp/population) as 'per capita GNP(USD)' FROM country WHERE continent = 'Europe';
-- 22. The number of years since independence for all countries that have a year of independence (192 rows)
SELECT name, (YEAR(GETDATE()) - indepyear) as 'years since independence' FROM country WHERE indepyear IS NOT NULL;